...
<div id="slideimage">
	...
	<img src="<?php the_field('logo',249); ?>">
	<div class="takasa30"></div>
</div>
...
<?php get_footer(); ?>
<script src="<?php echo esc_url( get_template_directory_uri() ); ?>/js/vegas.min.js"></script>
<script>
$(window).on('load resize', function(){
  $('#slideimage').vegas({ //背景画像でスライドショーしたい場所の設定
    slides: [
     { src: "<?php the_field('image01',34); ?>" }, //スライドする画像を配列で設定
     { src: "<?php the_field('image02',34); ?>" },
     { src: "<?php the_field('image03',34); ?>" },
     { src: "<?php the_field('image04',34); ?>" }
    ],
     delay: 5000, //スライドまでの時間ををミリ秒単位で設定
     timer: true, //タイマーバーの表示/非表示を切り替え
     overlay: "<?php echo home_url(); ?>/img/black.jpg", //オーバーレイする画像の設定
     animation: 'fade', //スライドのアニメーションを設定
